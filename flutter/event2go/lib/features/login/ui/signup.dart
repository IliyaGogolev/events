...
import 'package:event2go/features/login/domain/login_use_case.dart';
import 'package:event2go/features/login/ui/enter_code.dart';
...
class SignupPageState extends State<SignupWidget> {
  LoginUseCase loginUseCase; // = new SendPhoneNumberUseCase();
...
//    var signUpModel = new SignUpModel(child: widget);
    ...
    LoginRepository repo = new LoginRepositoryImpl(
        user: appModel.user, signUpModel: signUpModel);

    loginUseCase = new LoginUseCase(loginRepository: repo);
...
//      return '(###) ###-#### - Enter a US phone number.';
  ...
  void onLoginButtonClicked() {
    ...
    if (error == null) {
      loginUseCase.testVerifyPhoneNumber(phoneNumber,
...
//    if (loginUseCase.user.token != null && loginUseCase.user.token.isNotEmpty) {
    String token = loginUseCase.loginRepository.token;
    if (token != null && token.isNotEmpty) {
      print("testVerifyPhoneNumber finished, token " + token);

      // removes all of the routes except for the new pushed route HomeTabsView.tag
      Navigator.of(context)
          .pushNamedAndRemoveUntil(
          HomeTabsView.tag, (Route<dynamic> route) => false);
